...
fn edge(ps: &HashSet<(isize, isize)>) -> (HashSet<DirectedEdge>, HashSet<DirectedEdge>) {
  ...
  let interior_edges: HashSet<DirectedEdge> = boundary_edges
    ...
    .filter(|&edge| {
      ...
      let rotated_neighbor = (edge.from.0 - edge.dir.1, edge.from.1 + edge.dir.0); // Rotate 90 degrees
      boundary_edges.contains(&DirectedEdge {
        from: rotated_neighbor,
        dir: edge.dir,
      })
    })
    .cloned()
...
fn main() {
  ...
  for part in 0..=1 {
    ...
    for s in &unique_sets {
      ...
      if part == 0 {
        result += s.set.len() * boundary.len();
      } else {
        result += s.set.len() * interior.len();
      }
    }
    println!("{}", result);
  }
